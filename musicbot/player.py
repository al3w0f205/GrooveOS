...
class GuildMusicPlayer:
    ...
    - Prefetch N+1 (descarga siguiente mientras suena actual)
    """
    ...
    def __init__(
        ...
        ffmpeg_path: str,
        temp_root: str,
        on_state_change: Optional[Callable[[int], Awaitable[None]]] = None,  # callback(guild_id)
        on_track_started: Optional[Callable[[int, Track], Awaitable[None]]] = None,
        on_track_finished: Optional[Callable[[int, Track], Awaitable[None]]] = None,
    ):
        self.bot = bot
    ...
    def _ffmpeg_source(self, file_path: str) -> discord.FFmpegPCMAudio:
        ...
        # filtro leve: volumen/normalización suave (reduce cambios bruscos)
        # Puedes quitar loudnorm si prefieres menos CPU
    ...
    async def _prepare_track(self, track: Track, background: bool = False):
        """
        Resuelve info YouTube y descarga local.
        """
        async with self._download_lock:
            ...
            try:
                ...
                # dejamos track.title como estaba
                pass
    ...
    async def _play_current(self):
        ...
        if not self.current.temp_file or not os.path.exists(self.current.temp_file):
            ...
            self.current = None
            await self._notify_state()
            await self._advance_after_fail(finished)
            return
    ...
    async def _on_track_end(self, err: Optional[Exception]):
        ...
        # callback finished (para stats)
        if self.on_track_finished:
            try:
                await self.on_track_finished(self.guild_id, finished)
            except Exception:
                pass

        # Decidir siguiente según loops
        next_track: Optional[Track] = None

        if self.loop_track:
            next_track = finished  # reutilizamos mismo (no borramos archivo)
        else:
            # borrar archivo del que terminó
...
class MusicService:
    """
    Servicio que gestiona players por guild.
    """
